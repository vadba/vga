...
import { validationSchema } from "../CreateNewPost/utils/validationSchema.js";
...
const CreateNewPost = () => {
  ...
  const args = { category: false, postId: id };
  useEffect(() => {
    dispatch(fetchPostsData(args));
  }, [id]);
  ...
  const { posts, status } = useSelector((state) => state.postsCategorySlice);
  ...
  if (!!posts) {
    const initialValues = {
      ...
      inMain: !!posts.inMain ? posts.inMain : false,
      img: posts.img,
